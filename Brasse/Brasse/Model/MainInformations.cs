...
namespace Brasse.Model
{
    public class MainInformations
    {
        ...
        public static bool CheckUrl(string tryUrl)
        {
            ...
            {
                if (!Regex.IsMatch(tryUrl, @"(https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9][azA-Z0-9-]+[a-zA-Z0-9]\.[^\s]{2,}|www\.[a-zA-Z0-9][a-zA-Z0-9-]+[a-zA-Z09]\.[^\s]{2,}|https?:\/\/(?:www\.|(?!www))[a-zA-Z0-9]+\.[^\s]{2,}|www\.[a-zA-Z09]+\.[^\s]{2,})"))
                {
                    return false;
                }
                return true;
